...
import PreviewHistory from './preview-history-inner';
...
class WorkFlow extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      selectedCodes: [],
      previewNodeMap: props.previewNodeMap,
      currentClickNode: {},
    };
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    } else if (this.props.curGroupId !== nextProps.curGroupId) {
      const { nodes } = this.state;
      this.setState(
        {
          previewNodeMap: nextProps.previewNodeMap,
        },
        () => {
          this.renderGraph(nodes);
        },
  ...
  renderNode = (
    ...
    const { previewNodeMap = {} } = this.state;
    if (previewNodeMap) {
      // 为节点设置状态
    ...
    node.title = previewNodeMap[node.id]
